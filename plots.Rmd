...
mean_site_season<-aggregate(individualCount ~season+site_code, birds_sites_seasons_year, mean)

ggplot(mean_site_season, aes(fill=season, y=individualCount, x=site_code))+
geom_bar(position='dodge', stat='identity')+
  theme_classic()+
    xlab("Site")+
...
mean_year_season<-aggregate(individualCount ~season+year, birds_sites_seasons_year, mean)

ggplot(mean_year_season, aes(color=season, y=individualCount, x=year))+
  geom_point()+
geom_smooth(se=F)+
  theme_classic()+
    xlab("Year")+
...
birds_shrubs_km_winter<-full_join(birds_shrubs_winter,shrubs_by_area_km)%>%
  drop_na()
